...
- **通話**: 通話画面は **call.php に統合**。案内「私はホストです」は call.php の `.call-jitsi-host-hint` のみ。chat-main.css の `.call-video-container` / `.call-two-panels` 等はレガシー（チャットでは通話ウィンドウを出さないため未使用）。DOCS/CALL_CONNECT_AND_NOTIFICATION_FIX_PLAN.md・PHONE_VIDEO_CALL_PLAN.md 参照。
