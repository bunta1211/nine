...
- **通話（meet.jit.si 暫定案内）**: 相手パネル下に「接続しない場合は、画面内の『私はホストです』を押してください」を表示。`.call-jitsi-host-hint`（chat-main.css）。確実に繋ぐには自前 Jitsi で会議即開始設定を実施すること（DOCS/CALL_CONNECT_AND_NOTIFICATION_FIX_PLAN.md）。
