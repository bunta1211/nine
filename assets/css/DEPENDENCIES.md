...
| `panel-panels-unified.css` | パネル間空間の統一・page-chatレイアウト。上パネル共有4ページ（settings/design/tasks/notifications）の `body padding-top` と `.main-container` も管理。**body.page-login** も同様に 70px padding と main-container 高さを適用 | chat.php, settings.php, design.php, tasks.php, notifications.php, index.php |
...
| `login-landing.css` | ログイン画面（ランディング）専用。body.page-login の上パネルとメイン・メインと下端をくっつけた表示：`--header-height: 70px`（unified と統一）、`.main-container` は `height: calc(100vh - var(--header-height))` と `margin-top: -4px` で接着、中央パネルは縦 flex で `.login-landing-center` のみスクロール・`.login-landing-footer` を下端固定。背景グラデ・カード影・タイポグラフィ。768px 以下で縦積み。DOCS/LOGIN_LANDING_PLAN.md・STANDARD_DESIGN_SPEC.md「ログイン画面のレイアウト」参照 | index.php |
...
- **チャット入力テキストエリア**: 規格の詳細は **DOCS/STANDARD_DESIGN_SPEC.md** の「チャット入力欄のレイアウト規格」を参照。実装の参照先: `.input-row textarea`／`#messageInput` は **chat-main.css** と **chat-mobile.css**。**JS**（`includes/chat/scripts.php` の autoResizeInput / DOMContentLoaded）、**chat.php** のインライン style、**input-area-resize.js**（復元時168クランプ）、**.input-area**（chat-main.css）。チャット入力欄は**デフォルトで下端接着・上方向のみ拡大**。position/bottom は `body.page-chat .center-panel .input-area` で詳細度を確保する。**layout/center-panel.css は chat-new.css で import がコメントアウトされているためチャットでは読み込まれず**、テキストエリアの高さ変更は chat-main.css / chat-mobile.css で行う。長文貼り付けで入力欄・ボタンが消えないよう、**scripts.php** の `autoResizeInput`（貼り付け時は二重 rAF で最終クランプ）と **utils.js** の `Chat.utils.autoResizeInput` は計測時も **max-height を外さず**（常に cap を維持）scrollHeight を取得する。**scripts.php** の AI秘書用 input リスナーでは max-height: none / height: auto を設定せず cap を維持して計測し、送信後は `height = 'auto'` をやめ `autoResizeInput` を呼ぶ。**chat-main.css** で `.input-area` に `overflow: hidden` を指定（PC で主に効く防御。モバイルでは chat-mobile.css の `body.page-chat .input-area { overflow: visible !important }` が後から効くため、確実な防止は JS の max-height 維持に依存）。`.input-row` / `.input-wrapper` の `min-height: 168px` も継続使用。
- **枠線・ホバー・ドロップダウンの色**: 規格は **DOCS/STANDARD_DESIGN_SPEC.md** の「枠線・ボタン・ホバーの統一」「ドロップダウン・モーダル」を参照。
- **通話**: 通話画面は **call.php に統合**。call.php では「私はホストです」の案内は表示せず、繋がらない場合に**原因表示**（errorOccurred・15秒タイムアウト）と**ヘルプリンク**（help/call-troubleshooting.php）を表示する。chat-main.css の `.call-video-container` / `.call-two-panels` 等はレガシー（チャットでは通話ウィンドウを出さないため未使用）。DOCS/CALL_CONNECT_AND_NOTIFICATION_FIX_PLAN.md・PHONE_VIDEO_CALL_PLAN.md 参照。
...
**規格**: DOCS/STANDARD_DESIGN_SPEC.md の「ドロップダウン・モーダル」を参照。  
