...
### 手順1: マイグレーション用 PHP を本番に反映する

**実装ファイルを EC2 に直接アップロードはしません。** main にマージして push していれば、`database/run_ai_clone_migration.php` は GitHub Actions により EC2 の `/var/www/html/database/` に既に配置されています。まだの場合は先にコミット → main マージ → push を行ってください。
...
main にマージして push 済みで、EC2 に `run_ai_clone_migration.php` が配置されている前提です。EC2 に SSH してマイグレーションを実行するだけです。
...
マイグレーションは完了済みです。同じ手順を再実行するときは、main にマージ＋push 済みの前提で、EC2 に SSH して以下を実行します。
