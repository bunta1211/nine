...
├── design.php               ← デザイン設定画面
├── index.php                ← ログイン画面（未ログイン時は上パネル＋左・中央のランディング。左=ログインフォーム、中央=キャッチ・機能・用途・使い方。右パネルは廃止済み）
│
...
| グループメンバー | `api/friends.php?action=group_members` | DM開始用（個人アドレス帳モーダル） |
| ユーザー検索 | `api/users.php`, `api/friends.php` | グループ追加・個人アドレス帳検索 |
