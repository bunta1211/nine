...
| `access_logger.php` | アクセスログ記録・集計（log_page_access, get_access_stats_today, access_log_table_exists）。本日のアクセス・検索経由・離脱率。access_log テーブル利用。visitor_key は64文字以内。本日は PHP タイムゾーン（Asia/Tokyo）で集計。テーブル未作成時は管理画面で案内表示。config/database.php と config/app.php 前提 | index.php, chat.php, admin/index.php |
...
**ログイン画面**: index.php も同じ仕組み（`getCurrentLanguage()` / `setLanguage()` / `__($key)`）を使用。上パネルの言語切替（日本語・English・中文）で `?lang=ja|en|zh` に遷移するとセッションと Cookie（`social9_lang`）の両方に保存され、リダイレクト後に選択言語で表示される。Cookie によりセッションが切れても言語選択が保持される。
